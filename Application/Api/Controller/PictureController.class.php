...
class PictureController extends Controller
{
    ...
    protected function returnDefaultImage(){
...
//        $avatar = I('get.avatar','');
        ...
        if(!in_array($size,$this->accept_size)){
            $size = 0;
        }

        $Picture = D('UserPicture');
        $result = $Picture->where(array('id'=>$pic_id))->find();
...
//    public function convert2px($size){
//        $width =  0;
...
//    }
    ...
    protected function generate($info,$size){
        ...
        $save_name = $info['savename'].'.w'.$size;
        $image->thumb($size, $size,Image::IMAGE_THUMB_CENTER)->save($thumbnail_path.$save_name);
